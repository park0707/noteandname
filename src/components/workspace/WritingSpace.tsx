...
import SnapshotInputModal from './writingspace/Modals/SnapshotInputModal';
import AutoSaveModal from './writingspace/Modals/AutoSaveModal';

// Import shared editor utilities
import { formatSnapshotTimestamp } from './writingspace/editorUtils';
...
export default function WritingSpace(props: WritingSpaceProps) {
  ...
  return (
    <div className="flex-1 flex h-full w-full overflow-hidden relative" onScroll={preventScroll}>
      ...
      <SnapshotInputModal
        isDark={isDark}
        showSnapshotInputModal={showSnapshotInputModal}
        setShowSnapshotInputModal={setShowSnapshotInputModal}
        snapshotInputName={snapshotInputName}
        setSnapshotInputName={setSnapshotInputName}
        snapshotInputMemo={snapshotInputMemo}
        setSnapshotInputMemo={setSnapshotInputMemo}
        confirmCreateSnapshot={confirmCreateSnapshot}
      />

      {/* 6. 자동 스냅샷 저장 설정 모달 */}
      <AutoSaveModal
        isDark={isDark}
        showAutoSaveModal={showAutoSaveModal}
        setShowAutoSaveModal={setShowAutoSaveModal}
        tempWordsEnabled={tempWordsEnabled}
        setTempWordsEnabled={setTempWordsEnabled}
        tempWordsThreshold={tempWordsThreshold}
        setTempWordsThreshold={setTempWordsThreshold}
        tempTimeEnabled={tempTimeEnabled}
        setTempTimeEnabled={setTempTimeEnabled}
        tempTimeInterval={tempTimeInterval}
        setTempTimeInterval={setTempTimeInterval}
