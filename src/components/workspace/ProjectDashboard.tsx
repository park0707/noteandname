    ...
    <div className="flex-1 overflow-y-auto px-8 py-6 flex flex-col gap-6">
      ...
      <div className={`p-6 rounded-2xl border flex flex-col md:flex-row md:items-center justify-between gap-4 ${isDark ? 'bg-[#0D0E11] border-white/[0.06]' : 'bg-white border-black/[0.06]'
        ...
        <div className="flex flex-col">
          <span className={`text-[15px] ${isDark ? 'text-[#3A3D50]' : 'text-[#C5C5CC]'}`}>
          ...
          {editingName ? (
            <input
              ...
              className={`font-heading font-bold text-2xl mb-1.5 mt-1 bg-transparent border-b-2 border-[#5E6AD2] outline-none leading-tight w-full max-w-md ${isDark ? 'text-white' : 'text-[#121316]'}`}
            />
          ) : (
            <h2
              onClick={() => { setEditingNameValue(selectedProject.name); setEditingName(true); }}
              title="클릭하여 제목 수정"
              className={`font-heading font-bold text-2xl mb-1.5 mt-1 cursor-text select-none hover:opacity-70 transition-opacity ${isDark ? 'text-white' : 'text-[#121316]'}`}
            >
              {selectedProject.name}
            </h2>
          )}
          <p className={`text-sm pt-2 ${isDark ? 'text-[#A1A1AA]' : 'text-[#55555A]'}`}>
            ...
            <div className="flex flex-col gap-2">
              {mockChapters.map(ch => (
                <div key={ch.id} className={`flex items-center justify-between p-3 rounded-xl border transition-colors ${isDark ? 'border-white/[0.04] bg-white/[0.01] hover:bg-white/[0.03]' : 'border-black/[0.04] bg-black/[0.01] hover:bg-black/[0.03]'
                  ...
                  <button
                    onClick={() => setActiveFeature('editor')}
                    className={`text-xs px-3 py-1.5 rounded-lg border font-semibold transition-colors ${isDark ? 'border-white/[0.08] text-[#A1A1AA] hover:text-white hover:bg-white/[0.05]' : 'border-black/[0.08] text-[#55555A] hover:text-[#121316] hover:bg-black/[0.05]'
                      }`}
                  >
                    수정
                  </button>
