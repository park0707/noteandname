...
이 문서는 노벨플로우(Novelflow)의 최신 UI/UX 디자인 스펙 가이드라인입니다. Linear, Campfire, Notion 등 현대 시장을 선도하는 생산성 도구들의 핵심 디자인 트렌드를 분석하고, 이를 웹 및 데스크톱 Always-on-Top 미니 위젯에 적용하기 위한 최적의 디자인 스펙을 규정합니다.
...
- **선택 상태 피드백 (Selection Highlight)**:
  ...
  - 꼭짓점(Vertex) 편집 모드에서는 각 꼭짓점 위치에 지름 8px의 원형 앵커가 화이트(`#FFFFFF`) 바탕에 `--color-accent` 테두리로 표시되어 마우스 드래그 가이드를 제시합니다.
