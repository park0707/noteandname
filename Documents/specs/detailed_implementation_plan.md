...
## 5. 노션 양방향 실시간 동기화 (Notion Sync)

클라이언트의 설정 데이터가 노션 데이터베이스 백과 및 위키 페이지와 완전 동기화되는 OAuth 기반 커넥터를 활성화합니다.

### 5.1 OAuth 2.0 연동 및 연결 아키텍처
* **인증 절차**:
  - 노션 연동 버튼 클릭 시 `supabase.functions.invoke('notion-oauth')`를 호출하여 노션 인증 허브 창 오픈.
  - 사용자가 동기화를 원하는 노션 워크스페이스의 특정 페이지/데이터베이스 읽기/쓰기 권한을 허용하면 콜백 URL을 통해 안전하게 Access Token을 수신하여 Supabase Vault에 저장.
* **매핑 및 매뉴얼 바인딩 설정**:
  - 연동 성공 시 Notion Database의 필드 구조를 조회해 노벨플로우 캐릭터 테이블 구조와 대조 매핑.
  - 예: 노션의 `Name(Title)` 프로퍼티 ↔ 노벨플로우 `character.name`, 노션의 `Relation` 프로퍼티 ↔ 노벨플로우 `character_relations`.

### 5.2 초당 2회(2 rps) 속도 제어 Queue 파이프라인
* **통신 스펙**:
  - 노션 API의 엄격한 Rate Limit(초당 3회 권장, 실질적 2회 안전) 규격을 준수하기 위해 Deno Edge Function 단에서 **Redis 기반 분산 작업 대기열(Queue)**을 구동.
  - 사용자가 노벨플로우에서 다량의 원고/설정을 동기화하거나 노션에서 일괄 변경이 일어난 경우, 모든 쓰기 요청을 Queue에 등록하고 초당 최대 2건씩 디바운싱 처리하여 노션 서버로부터 429 Too Many Requests 예외를 사전 예방.
